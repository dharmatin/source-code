...
export default {
  ...
  searchSimilarityReferral: async(
    ...
    const conditionQ = `id:(${projectId}) AND (price_min_sort:[${priceMin} TO *] AND price_min_sort:[* TO ${priceMax}]) OR (price_max_sort:[${priceMin} TO *] AND price_max_sort:[* TO ${priceMax}])`;
    const queryListingById = listingClient.createQuery().q(conditionQ);
    return listingClient.searchAsync(queryListingById);
  },
};
