...
export const formatProject = (project: Object): Project => {
  ...
  const priceMax = !_.isNil(project.prices) ? parseFloat(project.prices[0].max) : 0;
  return {
    name: project.title,
    srcImage: !_.isNil(project.cover) ? project.cover.urlTemplate : '',
    priceRange: priceRangeFormat(priceMin, priceMax),
    provinceName: !_.isNil(project.multilanguagePlace) ? project.multilanguagePlace['en-GB'].level1 : '',
    cityName: !_.isNil(project.multilanguagePlace) ? project.multilanguagePlace['en-GB'].level2 : '',
    districtName: !_.isNil(project.multilanguagePlace) ? project.multilanguagePlace['en-GB'].level3 : '',
    urlReferral: project.shareLink,
    subUnitType: !_.isNil(project.propertyType) ? project.propertyType : '',
    builtUp: (!_.isNil(project.attributes) && !_.isNil(project.attributes.builtUp)) ? `${project.attributes.builtUp} ${config.translator.area_unit.sq_m}` : config.translator.contact_developer,
    landArea: (!_.isNil(project.attributes) && !_.isNil(project.attributes.landSize)) ? `${project.attributes.landSize} ${config.translator.area_unit.sq_ft}` : config.translator.contact_developer
  };
};
...
export const formatLister = (lister: Object): Lister => {
  return {
    name: lister.name,
    photoUrl: (!_.isNil(lister.image) && !_.isNil(lister.image.url)) ? lister.image.url : '',
    email: (!_.isNil(lister.contact) && !_.isNil(lister.contact.emails)) ? lister.contact.emails[0] : '',
    firstName: !_.isNil(lister.name) ? (lister.name).split(' ')[0] : '',
    profilePage: lister.website,
    membership: !_.isNil(lister.createdAt) ? membershipSince(new Date(lister.createdAt.substr(0, 10))) : ''
  };
};

export const formatOrganisation = (project: Object): Organisation => {
  return {
    name: (!_.isNil(project.organisations) && !_.isNil(project.organisations[0].name)) ? project.organisations[0].name : '',
    urlPage: (!_.isNil(project.organisations) && !_.isNil(project.organisations[0].website)) ? project.organisations[0].website : '',
    phoneNumber: (!_.isNil(project.organisations) && !_.isNil(project.organisations[0].contact.phones[0])) ? project.organisations[0].contact.phones[0].number : '',
